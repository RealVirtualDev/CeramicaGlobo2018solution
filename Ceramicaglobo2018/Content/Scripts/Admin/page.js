...
var editPage = function (pname, editLang) {

    let purl = "/admin/pages/getEditor"
    localStorage.clear();
    localStorage.lang = editLang;
    localStorage.pname = pname;

    // compilo pageeditor
    $.post(purl, { "pname": pname, "lang": editLang }, function (data) {
...
grid = $("#grid").jltable(
    {
        ...
        columns: [
            ...
            // COLONNA 0
            { title: "Sezione", fieldName: "sectionpath", columnType: "string", width: "auto", visible: true, columnName: "c1", style: { "text-align": "left", "padding-left": "20px" } },
            { title: "Pagina", fieldName: "titolo", columnType: "string", width: "auto", visible: true, columnName: "c2", style: { "text-align": "left", "padding-left": "20px" } },
        ...
        command: function (cname, carg, row) {
            ...
            switch (carg) {
                case 'elimina':
                    ...
                    // messagebox
                    $.mbox("Sicuri di voler eliminare la riga selezionata?",
                        {
                            icon: '/images/mbox/alert.png',
                            height: 100,
                            type: "custom",
                            buttons: [
                                { id: "mbox_custom_ok", link: "", text: "Si", cssclass: "mbox_button", localize: "bt_mbox_ok", command: true },
                                { id: "mbox_custom_gotocart", text: "No", cssclass: "mbox_button", localize: "btgotocart", command: false }
                            ],
                            buttonClicked: function (result) {
                                if (result) {
                                    // codice per eliminare la riga
                                    //alert("cancello la riga")
                                    // ribinda la griglia ricostruendo il datasource
